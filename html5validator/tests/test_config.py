...
HTML_TEST_FILES = Path(__file__).resolve().parent


def html5validator(config: Path) -> int:
    return subprocess.call(["html5validator", "--log=DEBUG", "--config", config])
...
def test_config_invalid_single_file():
    ...
    assert (
        html5validator(HTML_TEST_FILES / "config_files" / "invalid_single_file.yaml")
        != 0
    )
    ...
    assert (
        subprocess.call(
            [
                "html5validator",
                "--log=DEBUG",
                f"--config={HTML_TEST_FILES}/config_files/warning.yaml",
            ]
        )
        == 1
    )
...
def test_config_valid_format_flags():
    ...
    assert (
        html5validator(
            HTML_TEST_FILES / "config_files" / "format_flags" / "json_valid.yaml"
        )
        == 0
    )
    assert (
